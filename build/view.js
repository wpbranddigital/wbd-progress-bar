  ...
  function init() {
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        if (el.dataset.wbdMode === 'circular') animateRadial(el);else if (el.dataset.wbdMode === 'counter') animateCounterBlock(el);else animateBar(el);
    ...
    // Radial (scroll)
    document.querySelectorAll('[data-wbd-mode="circular"][data-wbd-animate="true"]').forEach(el => observer.observe(el));

    // Counters
    document.querySelectorAll('[data-wbd-mode="counter"]').forEach(el => {
      if (el.dataset.wbdAnimate === 'true') observer.observe(el);else animateCounterBlock(el);
    });

    // Linear bars + group rows (scroll)
    document.querySelectorAll('[data-wbd-animate="true"]:not([data-wbd-mode])').forEach(el => observer.observe(el));
  }
